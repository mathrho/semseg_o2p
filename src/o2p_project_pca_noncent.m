function o2p_project_pca_noncent(exp_dir, img_names, mask_type_train, mask_type_test, feat_collection, N_TO_KEEP)
    ...
    for h=1:numel(selection)
        ...
        if(strcmp(scaling_type, 'norm_2'))
            if(strcmp(feats{selection(h)}(1:6), 'Fisher'))
                feat_type_folder = strrep(feats{selection(h)}(7:end), '_', '');
                the_dir = [exp_dir 'MyFisherMat/' mask_type_test '/' feat_type_folder '_pca_' int2str(N_TO_KEEP(selection(h))) '_noncent/'];
            else
        ...
        elseif(isempty(scaling_type))
            if(strcmp(feats{selection(h)}(1:6), 'Fisher'))
                feat_type_folder = strrep(feats{selection(h)}(7:end), '_', '');
                the_dir = [exp_dir 'MyFisherMat/' mask_type_test '/' feat_type_folder '_pca_' int2str(N_TO_KEEP(selection(h))) '_nonorm_noncent/'];
            else
        ...
        for i=1:numel(img_names)
            if(~exist([the_dir img_names{i} '.mat'], 'file'))
                ...
                if(strcmp(feats{selection(h)}(1:6), 'Fisher'))
                    feat_type_folder = strrep(feats{selection(h)}(7:end), '_', '');
                    D = myload([exp_dir 'MyFisherMat/' mask_type_test '/' feat_type_folder '/' img_names{i} '.mat'], 'fish');
                else
