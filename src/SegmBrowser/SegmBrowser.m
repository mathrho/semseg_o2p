...
classdef SegmBrowser    
    ...
    methods
        ...
        function [Feats, dims, scaling] = get_whole_feats(obj, whole_ids, feat_types, scaling_type, weights, duh, lut, ndims)
          ...
          the_fisher_folder = 'MyFisherMat/';
            
          for i=1:numel(feat_types)
            if(strcmp(feat_types{i}(1:6), 'Fisher'))
              feat_type_folder = strrep(feat_types{i}(7:end), '_', '');
              D = myload([obj.exp_dir the_fisher_folder obj.mask_type '/' feat_type_folder ...
                  '/' obj.img_names{1} '.mat'], 'fish');
            else
          ...
          % get features strmatchfor each image, keep those for the requested wholes
          ...
          for j=1:numel(feat_types)
            ...
            for i=1:numel(un_img_ids)
              ...
              if(~lut)
                ...
                if(strcmp(feat_types{j}(1:6), 'Fisher'))
                  error('donot support lut for Fisher');
                else
                  D = myload([exp_dir the_folder mask_type '_' feat_types{j} '/' img_names{un_img_ids(i)} '.mat'], 'D');
        ...
        function [overlap] = get_overlaps_wholes(obj, whole_ids)
          % build overlaps for each segm per class
          ...
          for i=1:numel(un_img_ids)
            ...
            % get the max overlap among gts that with the same class
