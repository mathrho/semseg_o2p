function voc_test_models_later_fusion(feat_collections)
    addpath('../src/');
    ...
    DefaultVal('*feat_collections', '{''all_feats_pca_noncent_5000'', ''fisher_gmm_128''}');
    ...
    names = cell(size(feat_collections));
    if 1
      imgset_ho = 'val11';
      for i=1:numel(feat_collections)
        names{i} = [imgset_train '_' feat_collections{i} '_3.000000'];
      end
    else
      imgset_ho = 'test11';
      for i=1:numel(feat_collections)
    ...
    name = {strjoin(names, '_LATERFUSION_')};
    ...
    for h=1:numel(name)
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        y_preds = zeros(numel(feat_collections), 20, numel(whole_ho_ids));
        for g=1:numel(feat_collections)
            
            % set current feat_collection
            feat_collection = feat_collections{g};
            ...
            for i=1:numel(classes)            
                var = load([exp_dir 'MODELS/' names{g} '/' browser_ho.categories{classes(i)} '.mat']);
                beta(i,:) = var.model.w;
            end

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%% Get (class, segment) scores for each %%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            t_predicting = tic();
